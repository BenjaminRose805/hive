...
import { NO_WORKTREE_ROLES, type AgentsJson } from '../src/shared/agent-types.ts'
...
function withWorkerLock<T>(workerId: string, fn: () => Promise<T>): Promise<T> {
  ...
  workerLocks.set(workerId, next.then(() => {}, () => {}))
  return next
}
...
const DASHBOARD_CHANNEL_ID = GATEWAY_CONFIG_DATA?.dashboardChannelId ?? GATEWAY_CONFIG_DATA?.channelId ?? ''
...
async function isMentioned(msg: Message, mentionPatterns: string[], effectiveChannelId?: string): Promise<boolean> {
  // Direct @mention
...
async function routeInbound(msg: Message, excludeSender?: string): Promise<void> {
  ...
  for (const worker of workers.values()) {
    ...
    if (worker.role === 'manager') {
      const workerInfo: WorkerInfo = { workerId: worker.workerId, channelId: worker.channelId, role: worker.role }
  ...
  for (const worker of workers.values()) {
    ...
    if (mentioned) {
      ...
      if (decision.deliver) {
        targets.push(worker)
        targeted.add(worker.workerId) // Fix: dedup so Pass 3 skips Pass 2 targets
      }
    }
  }
  ...
  const deliveries = targets.map(async (worker) => {
    ...
    const isActiveMember = convo?.active.has(worker.workerId) && effectiveChannelId !== worker.channelId
    ...
    const inboxMsg: InboxMessage = {
      ...
      ...(parsed?.type === MessageType.TASK_ASSIGN && parsed.taskId && getChannelIdForTask(parsed.taskId)
    ...
    if (shouldNudge(worker) && !shouldDebounceNudge(worker.workerId)) {
      ...
      if (ok) {
        ...
        process.stderr.write(`hive-gateway: wrote to ${worker.workerId} inbox (nudge failed — message still in inbox)\n`)
      }
    } else {
...
function buildChannelXml(content: string, chatId: string, messageId: string, user: string, ts: string): string {
...
async function nudgeViaTmux(workerId: string): Promise<boolean> {
  return withWorkerLock(workerId, async () => {
    ...
    try {
      ...
      const load = Bun.spawn(['tmux', 'load-buffer', '-b', bufName, tmpFile], { stdout: 'pipe', stderr: 'pipe' })
...
async function ensureWorkerChannels(): Promise<Map<string, string>> {
  ...
  if (!dashboardCh || !('guild' in dashboardCh)) throw new Error('dashboard channel not in a guild')
  ...
  if (existsSync(channelsJsonPath)) {
    try {
      ...
      for (const worker of workers.values()) {
        ...
        if (!storedId) { allValid = false; break }
        try {
      ...
      if (allValid && channelMap.size === workers.size) {
        process.stderr.write(`hive-gateway: reusing ${channelMap.size} channels from previous run\n`)
        return channelMap
      }
    } catch {}
  }
  ...
  for (const worker of workers.values()) {
    let ch = allChannels.find(
      (c: any) => c?.type === ChannelType.GuildText && c.name === worker.workerId && c.parentId === category.id
    )
    if (!ch) {
...
async function createTaskChannel(taskId: string, description?: string, assignedAgent?: string): Promise<string> {
  ...
  const dashboardCh = await client.channels.fetch(DASHBOARD_CHANNEL_ID)
  if (!dashboardCh || !('guild' in dashboardCh)) throw new Error('Cannot find guild for task channel')
  const guild = (dashboardCh as any).guild
...
async function handleSlashSpinUp(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    ...
    if (isNoWorktreeRole) {
      ...
      const projectRepo = agentsData2?.agents.find(a => a.branch)?.branch?.replace(/^hive\//, '') ?? ''
      workDir = HIVE_ROOT
    } else {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (paneText.includes('text style') || paneText.includes('dark mode')) {
        ...
        Bun.spawnSync(['tmux', 'send-keys', '-t', `${SESSION_NAME}:${agentName}`, '', 'Enter'])
      } else if (paneText.includes('trust') || paneText.includes('syntax highlighting') || paneText.includes('get started')) {
        Bun.spawnSync(['tmux', 'send-keys', '-t', `${SESSION_NAME}:${agentName}`, '', 'Enter'])
    ...
    Bun.spawnSync(['tmux', 'send-keys', '-t', `${SESSION_NAME}:${agentName}`, initPrompt, 'Enter'], {
      stdout: 'pipe',
      stderr: 'pipe',
    })
    ...
    try {
      ...
      if (dashboardCh && 'guild' in dashboardCh) {
        ...
        if (category) {
          let ch = allChs.find(
            (c: any) => c?.type === ChannelType.GuildText && c.name === agentName && c.parentId === category.id
          )
          if (!ch) {
            ch = await guild.channels.create({ name: agentName, type: ChannelType.GuildText, parent: category.id })
          }
          const worker = workers.get(agentName)
    ...
    if (agentsData) {
      ...
      writeAgentsJson({
        agents: [{ name: agentName, role, domain, status: 'running', created: new Date().toISOString() }],
        created: new Date().toISOString(),
        mode: 'single-bot',
      })
    }
...
async function handleSlashTearDown(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    ...
    const SESSION_NAME = process.env.HIVE_SESSION ?? 'hive'
    Bun.spawnSync(['tmux', 'kill-window', '-t', `${SESSION_NAME}:${agentName}`], { stdout: 'pipe', stderr: 'pipe' })
...
async function handleSend(req: Request): Promise<Response> {
      ...
      switch (parsed.type) {
        case MessageType.STATUS: {
          ...
          if (['READY', 'ACCEPTED', 'IN_PROGRESS', 'BLOCKED', 'FAILED'].includes(status)) {
            const colors: Record<string, number> = { READY: 0x5865f2, ACCEPTED: 0x57f287, IN_PROGRESS: 0xfee75c, BLOCKED: 0xed4245, FAILED: 0xed4245 }
            embed = new EmbedBuilder()
              .setColor(colors[status] ?? 0x5865f2)
              .setDescription(`**${parsed.sender}** → \`${status}\`${parsed.taskId ? ` (${parsed.taskId})` : ''}${channelLink}`)
              .setTimestamp()
          }
          break
        }
        ...
        case MessageType.QUESTION: {
          embed = new EmbedBuilder()
            .setColor(0xfee75c)
            .setDescription(`**${parsed.sender}** has a question${parsed.taskId ? ` (${parsed.taskId})` : ''}${channelLink}`)
            .setTimestamp()
          break
        }
        case MessageType.ESCALATE: {
          embed = new EmbedBuilder()
            .setColor(0xed4245)
            .setDescription(`**${parsed.sender}** escalated${parsed.taskId ? ` (${parsed.taskId})` : ''}${channelLink}`)
            .setTimestamp()
          break
        }
      }
  ...
  const skipDirectInbox = targetAgent && conversationChannels.has(chatId)
...
async function handleNudge(req: Request): Promise<Response> {
  ...
  if (!shouldNudge(worker, priority)) {
    process.stderr.write(`hive-gateway: nudge suppressed for ${workerId} (${worker.status}, priority=${priority})\n`)
...
async function handleCreateChannel(req: Request): Promise<Response> {
  ...
  if (!Array.isArray(participants) || participants.length === 0) return jsonErr('participants required', 400)
  ...
  const dashboardCh = await client.channels.fetch(DASHBOARD_CHANNEL_ID)
  if (!dashboardCh || !('guild' in dashboardCh)) return jsonErr('Cannot find guild for conversation channel')
  const guild = (dashboardCh as any).guild
  const categoryId = GATEWAY_CONFIG_DATA?.categoryId
  ...
  if (message) {
    try {
      ...
      process.stderr.write(`hive-gateway: failed to post initial message to ${channelName}: ${e}\n`)
    }
  }
...
async function handleSetChannelTier(req: Request): Promise<Response> {
  ...
  if (!channelId) return jsonErr('channel_id required', 400)
  if (!agent) return jsonErr('agent required', 400)
  if (tier !== 'active' && tier !== 'observing') return jsonErr('tier must be active or observing', 400)
  ...
  try {
    ...
    try {
      ...
      if (existsSync(ccPath)) {
        ...
        process.stderr.write(`hive-gateway: loaded ${conversationChannels.size} conversation channel(s)\n`)
        ...
        try { agentsData = readAgentsJson() } catch {}
        ...
        persistConversationChannels()
        process.stderr.write(`hive-gateway: migrated ${conversationChannels.size} task channel(s) to conversation channels\n`)
      }
    } catch {}
  } catch (err) {
    const isPermission = String(err).includes('Missing Permissions') || String(err).includes('50013')
    if (isPermission) {
      process.stderr.write(`hive-gateway: WARNING — bot lacks Manage Channels permission. Falling back to single-channel mode.\n`)
      process.stderr.write(`hive-gateway: Grant the bot "Manage Channels" permission in your Discord server for per-worker channels.\n`)
