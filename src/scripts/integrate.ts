...
export async function dryRun(repo: string, workers: string[], target: string): Promise<void> {
  for (const worker of workers) {
    ...
    const existsResult = run(['git', 'rev-parse', '--verify', `refs/heads/${branch}`], { cwd: repo })
    ...
    const diffSummary = diff.stdout
      ? diff.stdout
          .split('\n')
          .filter(Boolean)
          .slice(-1)[0] // last line is the summary
      : '(no changes)'
...
export async function integrate(
  ...
  if (diffResult.exitCode !== 0 || diffCachedResult.exitCode !== 0) {
    console.error('ERROR: Working directory has uncommitted changes. Commit or stash before integrating.')
    process.exit(1)
  }
  ...
  for (const worker of workers) {
    ...
    if (mergeResult.exitCode !== 0) {
      ...
      if (autoResolve && conflictingFiles.length > 0) {
        ...
        const checkoutResult = run(
          ['git', 'checkout', '--theirs', ...conflictingFiles],
          { cwd: repo }
        )
        ...
        if (checkoutResult.exitCode !== 0 || addResult.exitCode !== 0 || commitResult.exitCode !== 0) {
