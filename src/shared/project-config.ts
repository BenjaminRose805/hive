...
export function resolveProject(config: HiveConfig, name: string): ProjectConfig & { repo: string; channel: string } {
...
export function listProjects(config: HiveConfig): void {
  ...
  for (const [name, proj] of projects) {
    ...
    console.log(`${name.padEnd(14)} ${repo.padEnd(36)} ${agents.padEnd(24)} ${merged.channel ?? '—'}`)
  }
}
