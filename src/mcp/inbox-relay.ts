...
import { existsSync, readdirSync, readFileSync, mkdirSync, unlinkSync, renameSync, statSync, writeFileSync } from 'fs'
...
const TOOLS = [
  {
    ...
    description: 'Read all pending messages from your Hive inbox. Returns an array of message objects and moves them to processed. Call this when you see a [hive] nudge notification.',
    inputSchema: {
  ...
  {
    name: 'hive__send',
    description: 'Send a message directly to another worker. Writes to their inbox and nudges them. The message is also echoed to Discord for human visibility.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        ...
        priority: {
          ...
          description: 'Message priority. info=FYI (default), alert=problem affecting them, response=answering their question, critical=must interrupt.',
        },
        task_id: {
          type: 'string',
          description: 'Optional task ID. When provided, the Discord echo goes to the task channel instead of the target agent channel.',
        },
      },
      required: ['to', 'text'],
    },
  },
  {
    ...
    description: 'Set your worker status. Use "available" when between tasks, "focused" when deep in work (suppresses non-critical nudges), "blocked" when waiting on something.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        status: {
  ...
  },
  {
    ...
    description: 'Create a conversation channel for multi-party discussion. You become active (inbox delivery). Other participants start as observing (they read Discord history and can promote themselves to active). Returns participant statuses.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        topic: { type: 'string', description: 'Channel topic/purpose (used in channel name)' },
        participants: { type: 'array', items: { type: 'string' }, description: 'Worker names to include (they start as observing)' },
  ...
  },
  {
    ...
    description: 'Add an agent to an existing conversation channel as an observer. They get a notification with the channel ID and can promote to active when ready. Returns their current status.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        channel_id: { type: 'string', description: 'Discord channel ID of the conversation channel' },
  ...
  },
  {
    ...
    description: 'Change your participation tier in a conversation channel. "active" = receive every message in your inbox (real-time collaboration). "observing" = no inbox delivery, read Discord history on your own schedule.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        channel_id: { type: 'string', description: 'Discord channel ID of the conversation channel' },
  ...
  },
  {
    ...
    description: 'Leave a conversation channel entirely. You will no longer receive messages or be listed as a participant.',
    inputSchema: {
      type: 'object' as const,
      properties: {
  ...
  },
  {
    ...
    description: 'List conversation channels you are a member of, with your tier (active/observing). Useful after context compaction to recover channel IDs.',
    inputSchema: {
      type: 'object' as const,
      properties: {},
      required: [],
  ...
  },
  {
    ...
    description: 'Get current status (available/focused/blocked) of all team members. Use before creating channels to check availability.',
    inputSchema: {
      type: 'object' as const,
      properties: {},
      required: [],
...
async function handleTool(name: string, args: Record<string, unknown>): Promise<string> {
  switch (name) {
    ...
    case 'hive__create_channel': {
      ...
      // MCP clients may stringify array arguments — parse if needed
      if (typeof participants === 'string') {
        try { participants = JSON.parse(participants) } catch { /* leave as-is, will fail validation below */ }
      }
    ...
    case 'hive__set_channel_tier': {
      ...
      if (!channelId) throw new Error('channel_id is required')
      if (!tier || !['active', 'observing'].includes(tier)) throw new Error('tier must be active or observing')
