...
const TOOLS = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        chat_id: {
          type: "string",
          description: "Discord channel ID (snowflake). Defaults to HIVE_CHANNEL_ID.",
        },
        text: {
          type: "string",
          description: "Message text to send (max 2000 chars per chunk, auto-split).",
        },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        emoji: {
          type: "string",
          description: "Emoji to react with (Unicode char or custom emoji ID).",
        },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        channel: {
          type: "string",
          description: "Channel ID to fetch from. Defaults to HIVE_CHANNEL_ID.",
        },
        limit: {
          type: "number",
        ...
        before: {
          type: "string",
          description: "Optional message ID — fetch messages before this one.",
        },
      },
    },
  },
...
async function main() {
  ...
  process.stderr.write(
    `discord-relay: MCP server started (worker=${WORKER_ID}, channel=${CHANNEL_ID})\n`,
  );
}
