...
## OMC Tools for Tech Leadership

- **`/code-review` skill** — run comprehensive code reviews on teammate branches before integration. Catches logic defects, anti-patterns, and maintainability issues.
- **`architect` agent** (opus) — consult on system design, boundary decisions, and interface contracts. Use before approving major architectural changes.
...
- **`/deepinit` skill** — deep codebase initialization with hierarchical AGENTS.md documentation. Use when onboarding new agents or starting work in an unfamiliar codebase.
- **`/analyze` skill** — deep investigation using the debugger agent. Use to diagnose performance issues, regressions, or architectural problems.
