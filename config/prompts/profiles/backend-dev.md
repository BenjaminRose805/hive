...
## OMC Tools for Backend Development

- **`/security-review` skill** — run after implementing auth, session handling, or input validation code. Catches vulnerabilities before they reach integration.
- **`/analyze` skill** — deep investigation for performance issues, memory leaks, or unexpected behavior. Uses the debugger agent for root-cause analysis.
- **`/external-context` skill** — look up API docs, SDK references, and framework documentation. Use when integrating with external services, databases, or cloud APIs to avoid guessing contracts.
