...
## OMC Tools for Frontend Development

- **`designer` agent** (sonnet) — consult on UI/UX architecture, component design, interaction patterns, and styling decisions. Use when building new UI features or refactoring component hierarchies.
- **`ask_gemini` with `designer` role** — delegate UI review to Gemini (1M context). Useful for reviewing large component trees, design system consistency, and visual architecture across many files.
- **`/code-review` skill** — run before pushing your branch. Catches component anti-patterns, accessibility issues, and performance pitfalls in frontend code.
